...
var transformCss_dist_vanillaExtractCssTransformCss = require('./transformCss-9f2e334d.browser.cjs.js');
...
var buildPropertyRule = _ref => {
  ...
  return transformCss_dist_vanillaExtractCssTransformCss._objectSpread2({
...
function extractLayerFromTokens(tokens) {
  if ('@layer' in tokens) {
    var {
      ...
      rest = transformCss_dist_vanillaExtractCssTransformCss._objectWithoutProperties(tokens, _excluded);
    return {
      layerName,
      tokens: rest
    };
...
function composedStyle(rules, debugId) {
  ...
  if (classList.length > 0) {
    result = "".concat(className, " ").concat(transformCss_dist_vanillaExtractCssTransformCss.dedupeAndJoinClassList(classList));
...
function composeStyles() {
  var compose = fileScope_dist_vanillaExtractCssFileScope.hasFileScope() ? composedStyle : transformCss_dist_vanillaExtractCssTransformCss.dedupeAndJoinClassList;
...
exports.assertVarName = assertVarName;
...
exports.createGlobalVar = createGlobalVar;
